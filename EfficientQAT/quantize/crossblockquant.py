...
def cross_block_quantization(
    ...
    for start_idx in range(0, num_layers, slide_step):
        ...
        if args.epochs > 0:
            ...
            quant_train_inps_loader = DataLoader(quant_train_inps, batch_size=args.batch_size, num_workers=4, pin_memory=True)
            fp_train_inps_loader = DataLoader(fp_train_inps, batch_size=args.batch_size, num_workers=4, pin_memory=True)
            quant_val_inps_loader = DataLoader(quant_val_inps, batch_size=args.batch_size, num_workers=4, pin_memory=True)
            fp_val_inps_loader = DataLoader(fp_val_inps, batch_size=args.batch_size, num_workers=4, pin_memory=True)
