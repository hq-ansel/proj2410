...
# 初始化分布式训练环境
def init_process(rank: int, size: int, fn: callable, backend: str = 'nccl', **kwargs) -> None:
    """
    初始化分布式训练环境
...
class TransformerLayers(nn.ModuleList):
...
@timer
def train_units_layers(rank: int, size: int, 
    ...
    # 解冻当前层，并冻结其它层
    # model.to(args.dev)
    total_training_iteration = args.epochs * args.train_size / args.batch_size / args.world_size
    step = 0
    param_groups = []
    param_group_index = 0
    ...
    layers = TransformerLayers(layers).to(rank)
    layers = DDP(layers, device_ids=[rank], )
    # 清除所有参数的梯度
    for p in layers.parameters():
        p.grad = None
    for qlayer in layers.module:
        ...
        loss_scaler= torch.amp.GradScaler(device=rank)
        trainable_number = trainable_parameters_num(layers)
        ...
        if hasattr(args,"dampen_loss"):
            dampen_loss_weight = args.get("dampen_loss_weight",0.0001)
        ...
        for epoch in range(args.epochs):
            ...
            for index, input_data in enumerate(trainloader):
                with torch.autocast(device_type=f"cuda:{rank}",
                    ...
                    loss = loss_func(hidden_state, target.to(rank,dtype=torch.float32))
                    
                    if args.get("dampen_loss",False):
                        dampen_loss = torch.zeros_like(loss).to(loss.device)
                        for idx, layer_idx in enumerate(trainable_layer_idx_list):
                            qlayer = layers.module[idx]
                ...
                if args.clip_grad > 0:
                        ...
                        norm = torch.nn.utils.clip_grad_norm_(trainable_parameters(layers), args.clip_grad).cpu()
                # 使用子空间优化
                if args.get("sub_space_grad_clean",False):
                    sub_space_clean(layers)
            ...
            # 假设你想计算的 loss_mean 是一个标量，在每个进程中独立计算的
            loss_mean = torch.stack(loss_list)[-(train_mean_num-1):].mean().to(rank)
            val_loss_list = []
            ...
            for index, input_data in enumerate(dataloader):  
                ...
                with torch.no_grad():
                    with torch.autocast(device_type=f"cuda:{rank}",
                                    ...
                                    dtype=args.dtype if amp_enabled else torch.float32):
                        inp,target = input_data
                        hidden_state = inp.to(f"cuda:{rank}",dtype=args.dtype)
                        ...
                        loss = loss_func(hidden_state, target.to(f"cuda:{rank}",dtype=torch.float32))
                val_loss_list.append(loss.cpu())
            val_loss_mean = torch.stack(val_loss_list).mean().to(rank)
            if rank == 0:
                # 创建一个列表用于在 rank 0 收集所有进程的 loss_mean
                gathered_loss_mean = [torch.zeros_like(loss_mean) for _ in range(size)]  # 创建一个列表来接收数据
                dist.gather(loss_mean, gathered_loss_mean, dst=0)  # 收集所有进程的 loss_mean
                # 将列表转换为张量
                gathered_loss_mean_tensor = torch.stack(gathered_loss_mean)
                
                gathered_val_loss_mean = [torch.zeros_like(val_loss_mean) for _ in range(size)]  # 创建一个列表来接收数据
                dist.gather(val_loss_mean, gathered_val_loss_mean, dst=0)  # 收集所有进程的 val_loss_mean
                # 将列表转换为张量
                gathered_val_loss_mean_tensor = torch.stack(gathered_val_loss_mean)

                # 计算平均值
                loss_mean = gathered_loss_mean_tensor.mean().item()
                val_loss_mean = gathered_val_loss_mean_tensor.mean().item()
                ...
                dist.gather(loss_mean, dst=0)  # 收集所有进程的 loss_mean
                dist.gather(val_loss_mean, dst=0)  # 收集所有进程的 val_loss_mean
        optimizer.zero_grad()
        del optimizer
...
def custom_shedule_train(model:PreTrainedModel,
    ...
    if args.get("with_catcher",False):
    ...
    for train_layer_window in shedule_list:
        ...
        if args.epochs > 0 :
            ...
            if len(skip_layers) > 0:
                ...
                skip_flag=False
            if  not skip_flag :
                if not args.get("with_catcher",False):

                    layers = [model.model.layers[i] for i in train_layer_window]
                    for layer in layers:
                        layer.to(dtype=args.dtype if amp_enabled else torch.float32)
                    # layers: List[nn.Module],
                    #     trainable_layer_idx_list: List[int],
                    #     loss_func: Callable,
                    #     train_dataset: LazyLoadDataset,
                    ...
                    train_units_layers(rank,world_size,layers=layers,
                    ...
                    # init_process(rank,world_size,train_units_layers,backend="nccl",
                    #              layers=layers,
                    #              trainable_layer_idx_list=train_layer_window,
                    #              loss_func=loss_func,
                    #              train_dataset=train_dataset,
                    ...
                    #              args=args,
                    #              )
                    for idx, i in enumerate(train_layer_window):
                        model.model.layers[i] = layers[idx].cpu()
                else: 
            ...
            if args.real_quant:
                ...
                for name, module in named_linears.items():
                    quantizer_version = getattr(
                        module, "quantizer_version", args.get("quantizer_version", "v1")
                    )
                    ...
                    print(f"pack quantized {name} with group_size {group_size}")
                ...
                with torch.no_grad():
                    with torch.autocast(device_type=f"cuda:{rank}",
                        ...
                        for slide_base in train_layer_window:
                            ...
                            layer = model.model.layers[layer_idx].to(f"cuda:{rank}",dtype=args.dtype)
                            next_layer = model.model.layers[layer_idx+args.slide_step].to(f"cuda:{rank}",dtype=args.dtype)
                            ...
                            model.model.layers[layer_idx+args.slide_step] = next_layer.to("cpu")
                            del layer, next_layer
                            gc.collect()
                            torch.cuda.empty_cache()
...
def greedy_local_train(
    ...
    assert isinstance(rank, int) and isinstance(world_size, int)
    ...
    model = model.cpu()
    if not args.get("use_catcher",False):
        class InputIDDataset(Dataset):
            def __init__(self, data):
                self.data = data
        ...
        train_dataset = InputIDDataset(trainloader)
        ...
        # 准备验证集
        val_dataset = InputIDDataset(valloader)
        sampler = torch.utils.data.distributed.DistributedSampler(val_dataset, num_replicas=world_size, rank=rank, shuffle=False)
        ...
        if train_dataset.attention_mask is None:
            ...
            attention_mask = train_dataset.attention_mask.to(f"cuda:{rank}")
        position_embeddings = (train_dataset.position_embeddings[0].to(f"cuda:{rank}"),
                            train_dataset.position_embeddings[1].to(f"cuda:{rank}"))
