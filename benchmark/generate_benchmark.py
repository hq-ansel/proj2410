...
def load_model_dispach(model_name, device="cuda"):
    if "EfficientQAT" in model_name:
        from EfficientQAT.quantize.int_linear_real import load_quantized_model
        wbits =2
        group_size = 128
        model,_ = load_quantized_model(model_name, wbits, group_size)
    elif "GPTQ" in model_name:
        from EfficientQAT.quantize.gptq_pipeline import load_model_and_tokenizer
        model, tokenizer = load_model_and_tokenizer(model_name)
    elif "aqlm" in model_name:
...
if __name__ == "__main__":
    ...
    if args.real_model:
        ...
        model = load_shared_model(args.model, device)

    prompt = torch.randint(low=0, high=model.config.vocab_size, size=(1, args.input_length), device=device)

    for i in range(args.warmup_iters + args.benchmark_iters):
        output = model.generate(prompt, min_new_tokens=args.output_length, max_new_tokens=args.output_length)
