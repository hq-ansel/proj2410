from transformers import AutoTokenizer,AutoformerModel
...
def get_wikitext2(tokenizer: AutoTokenizer, train_size: int, val_size: int, seed: int, seqlen: int, test_only: bool):
    """
    ...
    Args:
        ...
        test_only: if True, only return test data
    Returns:
        if test_only:
            testenc: a torch.tensor of shape (batch_size, seqlen) for test data
        else:
            trainloader: a list of tuples (input: torch.tensor, target: torch.tensor) for training
            valloader: a list of tuples (input: torch.tensor, target: torch.tensor) for validation
    """
    print("get_wikitext2")
...
def get_c4(tokenizer: AutoTokenizer, train_size: int, val_size: int, seed: int, seqlen: int, test_only: bool):
    """
    Get c4 dataset.
    Args:
        tokenizer: tokenizer for the dataset
...
def get_redpajama(tokenizer: AutoTokenizer, train_size: int, val_size: int, seed: int, seqlen: int):
    """
    Get redpajama dataset.
    Args:
        tokenizer: tokenizer for the dataset
        train_size: number of training examples
        val_size: number of validation examples
...
def get_loaders(
    name: str, tokenizer: AutoTokenizer, train_size=128, val_size=64, seed=0, seqlen=2048, test_only=False
):
    """
    Get data loaders for given dataset.
    Args:
...
@torch.no_grad()
def test_ppl(model: AutoformerModel, tokenizer: AutoTokenizer, datasets: list=['wikitext2'],ppl_seqlen=2048):
    """
    Test perplexity on given datasets.
    Args:
        model: the model to be tested
        tokenizer: tokenizer for the dataset
        datasets: a list of dataset names to be tested
        ppl_seqlen: maximum sequence length for perplexity calculation
    Returns:
        results: dict{dataset: ppl} a dictionary of perplexity for each dataset
    """
    results = {}
...
class BlockTrainDataset(Dataset):
    """
    A dataset for block training.
    derive from torch.utils.data.Dataset
    """
